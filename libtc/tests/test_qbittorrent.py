...
def client(request):
    with tempfile.TemporaryDirectory() as tmp_path:
        ...
        client = QBittorrentClient("http://localhost:8080/", "admin", "adminadmin", str(tmp_path / "qBittorrent"))
        for _ in range(30):
        ...
        client.call("post", '/api/v2/app/setPreferences', data={
            "json": json.dumps({"create_subfolder_enabled": request.param})
        })
        yield client
        if client.call("get", '/api/v2/app/preferences').json()['create_subfolder_enabled'] != request.param:
